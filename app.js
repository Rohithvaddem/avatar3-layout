    ...
    printWindow.document.write(`
        ...
        <body>
            <div class="quote-container">
                ...
                <!-- Custom Payment Plan Builder (Director Only) -->
                <div class="no-print" style="background: #f1f5f9; padding: 10px 14px; border-radius: 8px; margin-top: 12px; margin-bottom: 8px; border: 1px solid #cbd5e1; display: flex; align-items: center; justify-content: space-between; gap: 10px; flex-wrap: wrap;">
                    <div style="font-size: 12px; font-weight: 800; color: #0f172a; display: flex; align-items: center; gap: 6px;">
                        <i class="fa-solid fa-crown" style="color: #d97706;"></i> Director Payment Plan Builder:
                    </div>
                    <div style="display: flex; align-items: center; gap: 8px;">
                        <select id="selPaymentPlanPreset" style="padding: 5px 10px; border-radius: 6px; border: 1px solid #94a3b8; font-weight: 700; font-size: 12px; background: #fff; cursor: pointer; color: #0f172a;">
                            <option value="standard">Standard Plan (Booking + 25% + 100%)</option>
                            <option value="investor">VIP Investor Plan (Booking + 40% + 100%)</option>
                            <option value="express">50% Express Plan (Booking + 50% + 100%)</option>
                            <option value="custom">Director Custom Plan (Editable Dates & %)</option>
                        </select>
                    </div>
                </div>

                <!-- Payment Schedule Table -->
                <div class="section-header-banner" style="margin-top: 6px;" contenteditable="true">Payment Schedule</div>
                <table class="q-table">
                    <thead>
                        <tr>
            ...
            <script>
                function attachLiveRecalculation() {
                    ...
                    function updateCalculations() {
                        ...
                        const pct1El = document.getElementById('lblPct1');
                        const pct1Val = parseNum(pct1El ? pct1El.innerText : '25') / 100;
                        const bookingAmt = 100000;
                        const amt1 = Math.round(totalValueAmount * pct1Val);
                    ...
                    const planPresetEl = document.getElementById('selPaymentPlanPreset');
                    if (planPresetEl) {
                        planPresetEl.addEventListener('change', function() {
                            const plan = planPresetEl.value;
                            const lblPct1 = document.getElementById('lblPct1');
                            const lblScheduleLabel1 = document.getElementById('lblScheduleLabel1');
                            const lblScheduleLabel2 = document.getElementById('lblScheduleLabel2');

                            if (plan === 'standard') {
                                if (lblPct1) lblPct1.innerText = '25%';
                                if (lblScheduleLabel1) lblScheduleLabel1.innerText = 'WITHIN 15 DAYS';
                                if (lblScheduleLabel2) lblScheduleLabel2.innerText = 'WITHIN 45 DAYS';
                            } else if (plan === 'investor') {
                                if (lblPct1) lblPct1.innerText = '40%';
                                if (lblScheduleLabel1) lblScheduleLabel1.innerText = 'WITHIN 15 DAYS';
                                if (lblScheduleLabel2) lblScheduleLabel2.innerText = 'WITHIN 60 DAYS (REGISTRATION)';
                            } else if (plan === 'express') {
                                if (lblPct1) lblPct1.innerText = '50%';
                                if (lblScheduleLabel1) lblScheduleLabel1.innerText = 'WITHIN 30 DAYS';
                                if (lblScheduleLabel2) lblScheduleLabel2.innerText = 'WITHIN 60 DAYS';
                            }
                            updateCalculations();
                        });
                    }

                    ['lblPlotArea', 'lblClosingPrice', 'lblPerSqYd', 'lblOriginalTotalCost', 'lblDiscount', 'lblBankRate', 'lblCashRate', 'lblEastRate', 'lblCornerRate', 'lblMortgageRate', 'lblBankLoanRate', 'lblCorpusRate', 'lblPct1'].forEach(id => {
                        ...
                        if (el) {
                            ...
                            el.addEventListener('blur', function() {
                                ...
                                if (id !== 'lblPct1') el.innerText = fmt(val);
                                updateCalculations();
                            });
                        }
                    });
...
function openPlotModal(plotNo) {
    ...
    const releaseMortgageHtml = isDirectorLoggedIn && (plotStatusUpper === 'MORTGAGE' || plotStatusUpper === 'MORTAGAGE') ? `
        <button class="admin-login-btn" id="releaseMortgageBtn" style="background: linear-gradient(135deg, #f97316 0%, #ea580c 100%); color: #fff; border: none; font-weight: 700; width: 100%; margin-top: 10px; cursor: pointer; border-radius: 8px; display: flex; align-items: center; justify-content: center; gap: 8px; padding: 12px; font-size: 14px; box-shadow: 0 4px 14px rgba(249, 115, 22, 0.3);">
            <i class="fa-solid fa-key"></i> Release Mortgage Status to AVAILABLE
        </button>
    ` : '';
    ...
    const releaseMortgageBtn = document.getElementById('releaseMortgageBtn');
    if (releaseMortgageBtn) {
        releaseMortgageBtn.addEventListener('click', () => {
            if (confirm(`Are you sure you want to release Mortgage status for Plot #${plotNo} to AVAILABLE?`)) {
                item.plot_status = 'AVAILABLE';
                saveCustomData();
                updateStatistics();
                renderPlotDots();
                openPlotModal(plotNo);
                alert(`Plot #${plotNo} mortgage status has been successfully released to AVAILABLE!`);
            }
        });
    }
...
function generateExecutiveSummaryReport() {
    ...
    const projectMeta = projectMetadata[currentProject] || { title: currentProject === 'avatar1' ? "Aspirealty AVATAR 1" : "Aspirealty AVATAR 2" };
    const baseRate = (currentProject === 'avatar1') ? 14499 : 15499;
...
function setupAdminState() {
    ...
    if (isAdminLoggedIn) {
        ...
        const directorReportBtnHtml = isDirectorLoggedIn ? `
            <button class="admin-login-btn" id="executiveReportBtn" style="background: linear-gradient(135deg, #eab308 0%, #ca8a04 100%); color: #000; border: none; font-weight: 800; cursor: pointer; padding: 8px 10px; border-radius: 8px; display: flex; align-items: center; justify-content: center; gap: 6px; font-size: 11.5px; grid-column: span 2; box-shadow: 0 4px 12px rgba(234, 179, 8, 0.3);">
